...
<body class="flex min-h-screen bg-slate-50">
    ...
    <div class="mx-auto w-full max-w-[1440px] p-4 md:p-8">
        ...
        <div class="glass-card mb-8 p-6">
            <div class="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    <h3 class="text-base font-semibold text-slate-800">Department Attendance Statistics</h3>
                    <p class="text-xs text-slate-400">Today's breakdown of attendance status across all employees.</p>
                </div>
    ...
    <script>
        ...
        function buildDepartmentAttendanceData() {
            ...
            return {
                labels: departments,
                present: departments.map(department => attendanceRecords.filter(record => record.department === department && record.status === 'Present').length),
                late: departments.map(department => attendanceRecords.filter(record => record.department === department && record.status === 'Late').length),
                absent: departments.map(department => attendanceRecords.filter(record => record.department === department && record.status === 'Absent').length),
                outOfWindow: departments.map(department => attendanceRecords.filter(record => record.department === department && record.outOfWindow).length)
            };
        }
            ...
            departmentAttendanceChart = new Chart(departmentAttendanceChartCanvas, {
                ...
                data: {
                    ...
                    datasets: [
                        ...
                        {
                            label: 'Out of Window',
                            data: departmentData.outOfWindow,
