...
<head>
    ...
    <style>
        ...
        .leave-data-table tbody tr {
            background: rgba(255, 255, 255, 0.74);
            transition: background 0.2s ease, transform 0.2s ease;
        }

        .leave-data-table tbody tr:nth-child(even) {
            background: rgba(248, 250, 252, 0.92);
        }

        .leave-data-table tbody tr:hover {
...
<body class="flex min-h-screen bg-slate-50">
    ...
    <div class="mx-auto w-full max-w-[1440px] p-4 md:p-8">
        <div class="mb-8 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div class="grid w-full grid-cols-1 gap-2 sm:w-auto sm:grid-cols-3">
                <button id="openLeaveTypeModal" type="button" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-500 via-indigo-500 to-pink-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-100 transition hover:from-sky-600 hover:via-indigo-600 hover:to-pink-600">
                    <i class="fa-solid fa-plus text-xs"></i>
                    <span>Leave Type</span>
                </button>
                <button id="openNewLeaveModal" type="button" class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-semibold text-sky-700 transition hover:bg-sky-100">
                    <i class="fa-solid fa-calendar-plus text-xs"></i>
                    <span>New Leave</span>
                </button>
                <button id="openLeaveIssuanceModal" type="button" class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100">
                    <i class="fa-solid fa-list-check text-xs"></i>
                    <span>Leave Issuance</span>
                </button>
            </div>
        </div>
        ...
        <div class="glass-card mb-8 p-6">
            ...
            <div class="table-scroll overflow-x-auto">
                <table class="leave-data-table min-w-full text-left">
                    <thead>
    ...
    <!-- Leave Issuance Modal -->
    <div id="leaveIssuanceModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4">
        <div class="modal-backdrop absolute inset-0 bg-slate-900/40 backdrop-blur-sm"></div>
        <div class="glass-card relative z-10 flex w-full max-w-4xl flex-col overflow-hidden" style="max-height:90vh">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
                <div>
                    <h3 class="text-lg font-semibold text-slate-800">Leave Issuance</h3>
                    <p class="text-xs text-slate-400">Review and action pending leave requests from employees.</p>
                </div>
                <button type="button" id="closeLeaveIssuanceModal" class="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition hover:bg-slate-100">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <div class="overflow-y-auto p-6">
                <div id="leaveIssuanceList" class="space-y-3"></div>
            </div>
        </div>
    </div>

    <!-- New Leave Modal -->
    <div id="newLeaveModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4">
        <div class="modal-backdrop absolute inset-0 bg-slate-900/40 backdrop-blur-sm"></div>
        <div class="glass-card relative z-10 w-full max-w-lg overflow-hidden">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
                <div>
                    <h3 class="text-lg font-semibold text-slate-800">New Leave Request</h3>
                    <p class="text-xs text-slate-400">Issue a new leave on behalf of an employee.</p>
                </div>
                <button type="button" id="closeNewLeaveModal" class="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition hover:bg-slate-100">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <div class="max-h-[80vh] overflow-y-auto p-6">
                <form id="newLeaveForm" class="space-y-4">
                    <div>
                        <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Employee</label>
                        <select id="newLeaveEmployee" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100">
                            <option value="">Select employee...</option>
                            <option>Cabdiraxmaan Cali (EMP-0001)</option>
                            <option>Fadumo Xasan (EMP-0002)</option>
                            <option>Mahad Axmed (EMP-0003)</option>
                            <option>Sahra Maxamed (EMP-0004)</option>
                            <option>Hodan Ali (EMP-0005)</option>
                            <option>Mustafe Cabdi (EMP-0006)</option>
                            <option>Amina Yusuf (EMP-0007)</option>
                            <option>Roda Jama (EMP-0008)</option>
                        </select>
                    </div>
                    <div>
                        <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Leave Type</label>
                        <select id="newLeaveType" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Start Date</label>
                            <input type="date" id="newLeaveStartDate" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100">
                        </div>
                        <div>
                            <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">End Date</label>
                            <input type="date" id="newLeaveEndDate" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100">
                        </div>
                    </div>
                    <div>
                        <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Reason</label>
                        <textarea id="newLeaveReason" rows="3" placeholder="Brief reason for the leave request..." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 resize-none"></textarea>
                    </div>
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-4">
                        <button type="button" id="cancelNewLeaveModal" class="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">Cancel</button>
                        <button type="submit" class="rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-500">Submit Leave</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
                ...
                <tr class="transition-colors hover:bg-slate-50/70">
                    ...
                    <td class="px-4 py-4">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <button type="button" class="approve-leave-btn flex h-9 items-center gap-1.5 rounded-xl bg-emerald-50 px-3 text-xs font-semibold text-emerald-600 transition hover:bg-emerald-100" data-leave-index="${index}">
                                <i class="fa-solid fa-check text-xs"></i><span>Approve</span>
                            </button>
                            <button type="button" class="reject-leave-btn flex h-9 items-center gap-1.5 rounded-xl bg-rose-50 px-3 text-xs font-semibold text-rose-600 transition hover:bg-rose-100" data-leave-index="${index}">
                                <i class="fa-solid fa-xmark text-xs"></i><span>Reject</span>
                            </button>` : ''}
                        </div>
                    </td>
                </tr>
        ...
        leaveRequestsTableBody.addEventListener('click', event => {
            const viewTrigger    = event.target.closest('.view-leave-proof');
            const approveTrigger = event.target.closest('.approve-leave-btn');
            const rejectTrigger  = event.target.closest('.reject-leave-btn');

            if (viewTrigger) {
                showLeaveProof(Number(viewTrigger.dataset.leaveIndex));
                return;
            }
            if (approveTrigger) {
                const idx = Number(approveTrigger.dataset.leaveIndex);
            ...
            if (rejectTrigger) {
                const idx = Number(rejectTrigger.dataset.leaveIndex);
        ...
        // Leave Issuance Modal
        const leaveIssuanceModal   = document.getElementById('leaveIssuanceModal');
        const closeLeaveIssuanceModal = document.getElementById('closeLeaveIssuanceModal');
        const leaveIssuanceList    = document.getElementById('leaveIssuanceList');

        function renderLeaveIssuanceList() {
            const pending = leaveRequests.filter(r => r.status === 'Pending');
            if (!pending.length) {
                leaveIssuanceList.innerHTML = '<p class="py-8 text-center text-sm text-slate-400">No pending leave requests at this time.</p>';
                return;
            }
            leaveIssuanceList.innerHTML = pending.map((r, i) => {
                const globalIdx = leaveRequests.indexOf(r);
                return `
                <div class="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-4">
                        <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 font-bold text-sm">${r.employee.charAt(0)}</div>
                        <div>
                            <p class="text-sm font-semibold text-slate-800">${r.employee}</p>
                            <p class="text-xs text-slate-400">${r.leaveType} &middot; ${r.startDate} &rarr; ${r.endDate} (${r.days} days)</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <button type="button" class="issuance-approve-btn flex h-9 items-center gap-1.5 rounded-xl bg-emerald-50 px-4 text-xs font-semibold text-emerald-600 transition hover:bg-emerald-100" data-global-idx="${globalIdx}">
                            <i class="fa-solid fa-check text-xs"></i> Approve
                        </button>
                        <button type="button" class="issuance-reject-btn flex h-9 items-center gap-1.5 rounded-xl bg-rose-50 px-4 text-xs font-semibold text-rose-600 transition hover:bg-rose-100" data-global-idx="${globalIdx}">
        ...
        leaveIssuanceList.addEventListener('click', event => {
            const approveBtn = event.target.closest('.issuance-approve-btn');
            const rejectBtn  = event.target.closest('.issuance-reject-btn');
            if (approveBtn) {
                leaveRequests[Number(approveBtn.dataset.globalIdx)].status = 'Approved';
                renderLeaveIssuanceList();
                renderLeaveRequestsTable();
                renderInsightCards();
            }
            if (rejectBtn) {
                leaveRequests[Number(rejectBtn.dataset.globalIdx)].status = 'Rejected';
                renderLeaveIssuanceList();
                renderLeaveRequestsTable();
                renderInsightCards();
            }
        ...
        document.getElementById('openLeaveIssuanceModal').addEventListener('click', () => {
            renderLeaveIssuanceList();
            leaveIssuanceModal.classList.remove('hidden');
            leaveIssuanceModal.classList.add('flex');
            document.body.classList.add('overflow-hidden');
        });
        closeLeaveIssuanceModal.addEventListener('click', () => {
            leaveIssuanceModal.classList.add('hidden');
            leaveIssuanceModal.classList.remove('flex');
            document.body.classList.remove('overflow-hidden');
        });
        leaveIssuanceModal.addEventListener('click', e => {
            if (e.target === leaveIssuanceModal || e.target.classList.contains('modal-backdrop')) {
                closeLeaveIssuanceModal.click();
            }
        });

        // New Leave Modal
        const newLeaveModal    = document.getElementById('newLeaveModal');
        const closeNewLeaveModal = document.getElementById('closeNewLeaveModal');
        const cancelNewLeaveModal = document.getElementById('cancelNewLeaveModal');

        document.getElementById('openNewLeaveModal').addEventListener('click', () => {
            newLeaveModal.classList.remove('hidden');
            newLeaveModal.classList.add('flex');
            document.body.classList.add('overflow-hidden');
        });
        function closeNewLeave() {
            newLeaveModal.classList.add('hidden');
            newLeaveModal.classList.remove('flex');
            document.body.classList.remove('overflow-hidden');
        }
        closeNewLeaveModal.addEventListener('click', closeNewLeave);
        cancelNewLeaveModal.addEventListener('click', closeNewLeave);
        newLeaveModal.addEventListener('click', e => {
            if (e.target === newLeaveModal || e.target.classList.contains('modal-backdrop')) closeNewLeave();
        });

        document.getElementById('newLeaveForm').addEventListener('submit', e => {
            e.preventDefault();
            const employee  = document.getElementById('newLeaveEmployee').value || 'Unknown Employee';
            const leaveType = document.getElementById('newLeaveType').value;
            const startDate = document.getElementById('newLeaveStartDate').value || '2026-05-01';
            const endDate   = document.getElementById('newLeaveEndDate').value || '2026-05-03';
            const start     = new Date(startDate);
            const end       = new Date(endDate);
            const days      = Math.max(1, Math.round((end - start) / 86400000) + 1);
            leaveRequests.push({
                employee: employee.replace(/ \(EMP-\d+\)/, ''),
                department: 'General',
                leaveType,
                startDate,
                endDate,
