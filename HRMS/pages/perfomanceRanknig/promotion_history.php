...
<body class="flex min-h-screen bg-slate-50">
    ...
    <script>
        ...
        function renderPromotionHistoryTable(records) {
            promotionHistoryTableBody.innerHTML = records.map((record, index) => `
                <tr class="transition-colors hover:bg-slate-50/70">
                    ...
                    <td class="px-5 py-4">
                        <button type="button" class="view-promotion-history inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-50" data-record-index="${index}">
        ...
        function showPromotionHistoryDetail(record) {
            let modal = document.getElementById('promotionHistoryDetailModal');

            if (!modal) {
                document.body.insertAdjacentHTML('beforeend', `
                    <div id="promotionHistoryDetailModal" class="modal-backdrop fixed inset-0 z-50 hidden items-center justify-center p-4">
                        <div class="glass-card w-full max-w-3xl overflow-hidden">
                            <div class="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
                                <div>
                                    <h3 id="promotionHistoryDetailTitle" class="text-lg font-semibold text-slate-800"></h3>
                                    <p id="promotionHistoryDetailSubtitle" class="mt-1 text-sm text-slate-500"></p>
                                </div>
                                <button id="closePromotionHistoryDetailModal" type="button" class="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-800">
                                    <i class="fa-solid fa-xmark"></i>
                                </button>
                            </div>
                            <div id="promotionHistoryDetailBody" class="grid gap-3 p-6 sm:grid-cols-2"></div>
                        </div>
                    </div>
                `);

                modal = document.getElementById('promotionHistoryDetailModal');
                const closeModal = () => {
                    modal.classList.add('hidden');
                    modal.classList.remove('flex');
                    document.body.classList.remove('overflow-hidden');
                };
                document.getElementById('closePromotionHistoryDetailModal').addEventListener('click', closeModal);
            ...
            const rows = [
                ...
                ['Previous Role', record.previousRole],
                ['New Role', record.newRole],
                ['Previous Rank / Grade', record.previousRankGrade],
                ['New Rank / Grade', record.newRankGrade],
                ['Effective Date', record.effectiveDate],
                ['Reason', record.reason],
                ['Approved By', record.approvedBy],
                ['Months To Promotion', `${record.monthsToPromotion} Months`],
                ['Notes', record.notes]
            ];

            document.getElementById('promotionHistoryDetailTitle').textContent = record.employee;
            document.getElementById('promotionHistoryDetailSubtitle').textContent = 'Promotion history detail';
            document.getElementById('promotionHistoryDetailBody').innerHTML = rows.map(([label, value]) => `
        ...
        promotionHistoryTableBody.addEventListener('click', event => {
            const trigger = event.target.closest('.view-promotion-history');

            if (!trigger) {
                return;
            }

            const record = getFilteredPromotionHistory()[Number(trigger.dataset.recordIndex)];

            if (record) {
                showPromotionHistoryDetail(record);
            }
        });
